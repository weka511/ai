...
class CRPdd:
    ...
    def _adjust_allocations(self,removed_cluster_number,detached_points):
        '''
        This method is used to update allocation when we have deleted a cluster
...
class EstablishStylesUsingCRP(Stage2):
    ...
    def _execute(self):
        ...
        for iclass in range(n_classes):
            crp = ChineseRestaurantProcess(self.mutual_information[iclass,:,:],
                                           alpha=self.alphs,
                                           rng=self.rng,
                                           logger=self.logger)
            crp.build()
